"""minijev POC experiments. Run from poc/:
    ...
    llm_vs_minijev  E10: the same model *writing* its answer (label / JSON) vs minijev *reading* it:
                 AG News accuracy + calibration + speed, and fan-out over 1 / 4 / 13 questions
    fanout       just the fan-out half of llm_vs_minijev
...
from minijev_poc import CONTENT_FREE_STATE, MODEL, MODES, SYSTEM, Engine, answer, ask, raw_scores
    ...
    for label, state, q, jev in JEV_DOC_CASES:
        ...
        for mode in ("listwise", "pointwise"):
            ...
            for r in range(len(keys)):
                ...
                dists.append(ask(engine, {"state": state, "questions": {"q": qq}}, debug=True)["answers"]["q"]["probabilities"])
...
def llm_vs_minijev(engine: Engine, n: int = 120, n_json: int = 60, state_tokens: int = 500) -> dict:
    ...
    for i, ex in enumerate(ag_news(n)):
        ...
        if i < n_json:
            ...
            calls.append(g_probs)
            parsed = parse_json(g_probs["text"]) or {}
            verbal = [max(0.0, number(parsed.get(k))) for k in keys]
            row["generate_json_probs"] = {"probs": verbal if sum(verbal) > 0 else None, "text": g_probs["text"],
                                          "seconds": g_probs["seconds"], "new_tokens": g_probs["new_tokens"],
                                          "raw_sum": sum(verbal)}
        rows.append(row)
        if (i + 1) % 25 == 0:
    ...
    readout = multiclass_metrics([r["readout"]["probs"] for r in rows], labels)
    readout_sub = multiclass_metrics([r["readout"]["probs"] for r in rows[:n_json]], labels[:n_json])
    label_answers = [r["generate_label"]["answer"] for r in rows]
    json_rows = [r["generate_json_probs"] for r in rows[:n_json]]
    json_probs = [j["probs"] for j in json_rows]
    json_norm = [[v / sum(p) for v in p] if p else uniform for p in json_probs]  # for ECE/Brier: unparsed = no info
    json_correct = sum(p is not None and max(range(len(p)), key=p.__getitem__) == y
                       for p, y in zip(json_probs, labels[:n_json]))
    single = {
        "readout (minijev)": {**readout, "n": n, "parse_failures": 0,
                              "mean_s": statistics.mean(r["readout"]["seconds"] for r in rows),
                              "output_tokens": 0},
        "generate label": {"accuracy": sum(a == keys[y] for a, y in zip(label_answers, labels)) / n,
                           "ece": None, "brier": None, "n": n,
                           "parse_failures": sum(a is None for a in label_answers),
                           "mean_s": statistics.mean(r["generate_label"]["seconds"] for r in rows),
                           "output_tokens": statistics.mean(r["generate_label"]["new_tokens"] for r in rows)},
        f"readout (minijev), first {n_json}": {**readout_sub, "n": n_json, "parse_failures": 0,
                                               "mean_s": statistics.mean(r["readout"]["seconds"] for r in rows[:n_json]),
                                               "output_tokens": 0},
        "generate JSON probabilities": {**multiclass_metrics(json_norm, labels[:n_json]),
                                        "accuracy": json_correct / n_json,  # unparsed counts as wrong
                                        "n": n_json, "parse_failures": sum(p is None for p in json_probs),
                                        "mean_s": statistics.mean(j["seconds"] for j in json_rows),
                                        "output_tokens": statistics.mean(j["new_tokens"] for j in json_rows)},
    }
    readout_ms_per_token = 1000 * sum(r["readout"]["seconds"] for r in rows) / sum(r["readout"]["input_tokens"] for r in rows)
    readout_pick = [keys[max(range(len(keys)), key=r["readout"]["probs"].__getitem__)] for r in rows]
    agreement = sum(a == b for a, b in zip(label_answers, readout_pick)) / n

    # Where generation spends its time: fit seconds ≈ a + b·prompt_tokens + c·new_tokens.
    X = np.array([[1.0, c["prompt_tokens"], c["new_tokens"]] for c in calls])
    y = np.array([c["seconds"] for c in calls])
    a, b, c = np.linalg.lstsq(X, y, rcond=None)[0]
    costs = {"fixed_ms": 1000 * a, "ms_per_prompt_token": 1000 * b, "ms_per_generated_token": 1000 * c,
             "readout_ms_per_input_token": readout_ms_per_token}

    print("\nAG News (same model, same CPU). Accuracy by chance = 0.25.")
    print(f"{'method':34s} {'n':>4s} {'acc':>6s} {'ECE':>6s} {'Brier':>6s} {'fails':>6s} {'mean s':>7s} {'out tok':>8s}")
    for name, m in single.items():
        fmt = lambda v: f"{v:6.3f}" if isinstance(v, float) else f"{'—':>6s}"
        print(f"{name:34s} {m['n']:4d} {fmt(m['accuracy'])} {fmt(m['ece'])} {fmt(m['brier'])} "
              f"{m['parse_failures']:6d} {m['mean_s']:7.2f} {m['output_tokens']:8.1f}")
    print(f"generated label == minijev argmax on {agreement:.1%} of items")
    print(f"cost on this CPU: generating {costs['ms_per_generated_token']:.0f} ms per output token "
          f"(regression over {len(calls)} completions); reading {readout_ms_per_token:.1f} ms per input token")

    fanout = fan_out(engine, state_tokens)
    return {"n": n, "single_decision": single, "label_vs_readout_agreement": agreement, "generation_costs": costs,
            "fan_out": fanout, "rows": rows}
...
def fan_out(engine: Engine, state_tokens: int, counts=(1, 4, 13)) -> list[dict]:
    """Many questions about one document: N separate completions, one JSON completion, or one minijev pass."""
    ...
    out = []
    print(f"\nFan-out: GDPR article cut to {state_tokens} tokens, Choice questions (same model, same CPU)")
    for n in counts:
        qs = dict(list(questions.items())[:n])
        # 1) one completion per question: the state is re-read every time
        t0, per_q, tokens = time.perf_counter(), {}, 0
        for qid, q in qs.items():
            g = generate(engine, f"STATE:\n{doc}\n\nQUESTION: {q['instructions']}\nOPTIONS:\n"
                                 f"{options_text(q['criteria'])}\nAnswer with the name of the best option only.", 12)
            per_q[qid], tokens = match_option(g["text"], list(q["criteria"])), tokens + g["new_tokens"]
        t_per_q = time.perf_counter() - t0
        # 1b) the same, with prompt caching: the document is read once and its KV cache reused per question
        prefix, cache = engine.prefix_ids(state), DynamicCache()
        t0, cached, tokens_cached = time.perf_counter(), {}, 0
        with torch.inference_mode():
            engine.model(torch.tensor([prefix]), past_key_values=cache, use_cache=True, logits_to_keep=1)
            for qid, q in qs.items():
                x = torch.tensor([prefix + engine.suffix_ids(
                ...
                cached[qid] = match_option(engine.tok.decode(new, skip_special_tokens=True), list(q["criteria"]))
                tokens_cached += len(new)
                cache.crop(len(prefix))  # back to the shared document for the next question
        t_cached = time.perf_counter() - t0
        # 2) one completion that answers every question as JSON
        listing = "\n\n".join(f"q{i}: {q['instructions']}\nOPTIONS:\n{options_text(q['criteria'])}"
                              for i, q in enumerate(qs.values(), start=1))
        g = generate(engine, f"STATE:\n{doc}\n\nAnswer each question below by choosing one of its options.\n\n"
                             f"{listing}\n\nReply with a JSON object that maps each question id (q1, q2, ...) to the "
                             f"name of the chosen option. Reply with the JSON object only.", 24 * n + 40)
        parsed = parse_json(g["text"]) or {}
        one_json = {qid: match_option(str(parsed.get(f"q{i}", "")), list(q["criteria"]))
                    for i, (qid, q) in enumerate(qs.items(), start=1)}
        # 3) minijev: one packed pass, one readout per question
        t0 = time.perf_counter()
        raw, usage = raw_scores(engine, {"state": state, "questions": qs}, "packed")
        t_readout = time.perf_counter() - t0
        picks = {qid: answer(q, raw[qid]["logits"])["choice"] for qid, q in qs.items()}
        row = {
            "questions": n,
            "generate_per_question": {"seconds": t_per_q, "output_tokens": tokens,
                                      "parse_failures": sum(v is None for v in per_q.values()),
                                      "agrees_with_readout": sum(per_q[k] == picks[k] for k in qs) / n},
            "generate_per_question_cached": {"seconds": t_cached, "output_tokens": tokens_cached,
                                             "parse_failures": sum(v is None for v in cached.values()),
                                             "agrees_with_readout": sum(cached[k] == picks[k] for k in qs) / n,
                                             "agrees_with_uncached": sum(cached[k] == per_q[k] for k in qs) / n},
            "generate_one_json": {"seconds": g["seconds"], "output_tokens": g["new_tokens"],
                                  "parse_failures": sum(v is None for v in one_json.values()),
                                  "agrees_with_readout": sum(one_json[k] == picks[k] for k in qs) / n},
            "readout_packed": {"seconds": t_readout, "input_tokens": usage["input_tokens"]},
        }
        out.append(row)
        print(f"  {n:2d} questions: per-question generation {t_per_q:6.2f}s ({tokens} tokens out)  |  "
              f"same with cached document {t_cached:6.2f}s  |  "
              f"one JSON generation {g['seconds']:6.2f}s ({g['new_tokens']} tokens out, "
              f"{row['generate_one_json']['parse_failures']} unparsed)  |  minijev readout {t_readout:6.2f}s")
    return out
...
def calibration(engine: Engine, n: int = 400) -> dict:
    ...
    if cache.exists() and json.loads(cache.read_text())["n"] == n:
        ...
        cache.write_text(json.dumps({"n": n, "raw": z_raw, "content_free": z_cf, "labels": y}))
    ...
    print(f"BoolQ dev, n={n}, base rate yes={sum(y) / n:.3f}")
    print(f"{'variant':36s} {'acc':>6s} {'ECE':>6s} {'Brier':>6s} {'NLL':>6s} {'mean p':>7s}")
    for name, m in table.items():
        print(f"{name:36s} {m['accuracy']:6.3f} {m['ece']:6.3f} {m['brier']:6.3f} {m['nll']:6.3f} {m['mean_p_yes']:7.3f}")
