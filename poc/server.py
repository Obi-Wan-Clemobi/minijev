"""minijev local API for the web playground. Run from poc/:

    uv run uvicorn server:app --port 8000

One Engine is loaded at startup. A lock serializes forward passes: the model runs on the CPU, one
request at a time. Every endpoint reuses the POC code; nothing here changes how answers are made.
"""
...
def v1_compare(req: CompareReq):
    ...
    with _lock, torch.inference_mode():
        ...
        def readout():
            from minijev_poc import raw_scores
