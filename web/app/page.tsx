...
import { blankQuestion, problems, QuestionCard } from "@/components/QuestionEditor";
...
export default function Playground() {
  ...
  const blocked = Object.keys(issues).length > 0 || s.items.length === 0 || !s.stateText.trim();
  const stateIsJson = typeof textToState(s.stateText) !== "string";

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === "Enter" && !blocked && !s.running) { e.preventDefault(); s.run(); }
    };
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="min-h-16 px-4 md:px-8 py-3 border-b border-line flex items-center gap-3 flex-wrap">
        <span className="text-[13px] text-muted">Preset</span>
        ...
        <button onClick={s.run} disabled={blocked || s.running}
          ...
          <Icon name="play" size={14} fill />{s.running ? "Running…" : "Run"}<span className="font-mono text-[11px] opacity-60">⌘↵</span>
        </button>
      </div>

      <div className="flex-1 grid lg:grid-cols-[minmax(460px,560px)_minmax(0,1fr)]">
        <div className="lg:border-r border-line px-4 md:px-8 py-6 flex flex-col gap-5">
          ...
          {view === "json" ? (
            ...
            <>
              <div className="flex flex-col gap-2">
                <label htmlFor="state" className="text-xs font-medium text-muted uppercase tracking-wider">State</label>
